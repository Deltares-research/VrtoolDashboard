...
from tests.test_callbacks.test_callback_optimize import (
...
class TestRunOptimize:
    ...
    def test_traject_optimization_table(self):
        ...
        expected_keys = [
            ...
            "reference_year",
            "GROUND_IMPROVEMENT",
            "GROUND_IMPROVEMENT_WITH_STABILITY_SCREEN",
            "GEOTEXTILE",
            "DIAPHRAGM_WALL",
            "STABILITY_SCREEN",
        ]
        ...
        assert all(
            [
                isinstance(row_1[key], bool)
                for key in expected_keys
                if key not in ["section_col", "reference_year"]
            ]
        )
    ...
    def test_get_selected_measure(self):
        ...
        _test_input_directory = Path.joinpath(
            test_data, "38-1 base river case", "config.json"
        )
    ...
    def test_run_optimize_no_revetment(self):
        ...
        shutil.copyfile(
            Path.joinpath(_vr_config.input_directory, vr_config["input_database_name"]),
            Path.joinpath(
                _vr_config.input_directory, "copy_" + vr_config["input_database_name"]
            ),
        )
    ...
    def test_run_optimize_with_revetment(self):
        ...
        _test_input_directory = Path.joinpath(
            test_data, "31-1 base coastal case", "config.json"
        )
