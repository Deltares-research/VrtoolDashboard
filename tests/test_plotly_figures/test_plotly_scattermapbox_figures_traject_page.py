...
from src.plotly_graphs.plotly_maps import (
    dike_traject_pf_cost_helping_map_detail,
...
class TestPlotlyScatterMapBox:
    ...
    @pytest.mark.parametrize(
        "result_type", [ResultType.RELIABILITY, ResultType.PROBABILITY]
    )
    @pytest.mark.parametrize(
        "mechanism",
        [Mechanism.SECTION, Mechanism.PIPING, Mechanism.OVERFLOW, Mechanism.STABILITY],
    )
    def test_plot_dike_traject_reliability_initial_assessment_map(
        self, result_type: Enum, mechanism: Enum
    ):
        # 1. Define data
        _dike_data = json.load(
            open(
        ...
        _fig = plot_dike_traject_reliability_initial_assessment_map(
            _dike_traject, 2025, result_type.name, mechanism.name
        )
        # 3. Assert
        assert isinstance(_fig, Figure)
    ...
    @pytest.mark.parametrize(
        "calc_type", [CalcType.VEILIGHEIDSRENDEMENT, CalcType.DOORSNEDE_EISEN]
    )
    @pytest.mark.parametrize(
        "sub_result_type", [SubResultType.ABSOLUTE, SubResultType.RATIO]
    )
    def test_plot_dike_traject_reliability_measures_assessment_map_reliability(
        self, result_type: Enum, calc_type: Enum, sub_result_type: Enum
    ):
        # 1. Define data
        _dike_data = json.load(
            open(
    ...
    @pytest.mark.parametrize(
        "sub_result_type", [SubResultType.ABSOLUTE, SubResultType.DIFFERENCE]
    )
    def test_plot_dike_traject_reliability_measures_assessment_map_cost(
        self, result_type: Enum, calc_type: Enum, sub_result_type: Enum
    ):
        # 1. Define data
        _dike_data = json.load(
    ...
    @pytest.mark.parametrize(
        "sub_result_type",
        [
            SubResultType.MEASURE_TYPE,
            SubResultType.BERM_WIDENING,
            SubResultType.CREST_HIGHTENING,
            SubResultType.INVESTMENT_YEAR,
        ],
    )
    def test_plot_dike_traject_reliability_measures_assessment_map_measure(
        self, calc_type: Enum, sub_result_type: Enum
    ):
        # 1. Define data
        _dike_data = json.load(
            open(
    ...
    def test_dike_traject_pf_cost_helping_map_simple(self):
        ...
        _fig2 = dike_traject_pf_cost_helping_map_simple(
            _dike_traject, 0, ["93"]
        )  # this one has a grey dijkvak
    ...
    def test_dike_traject_pf_cost_helping_map_detailed(self):
        ...
        # 2. Call
        _fig1 = dike_traject_pf_cost_helping_map_detail(
            _dike_traject,
            0,
            [
