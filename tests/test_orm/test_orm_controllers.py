...
class TestOrmControllers:
    ...
    def test_initialization_get_dike_traject_from_ORM(self):
        _vr_config = VrtoolConfig().from_json(
            ...
            / "data/TestCase1_38-1_no_housing/vr_config.json"
        )
        _vr_config.input_directory = (
            Path(__file__).parent.parent / "data/TestCase1_38-1_no_housing"
        )
    ...
    def test_get_dike_traject_from_ORM_returns_correct_dike_traject(self):
        # 1. Define data
        with open(
            Path(__file__).parent.parent / "data/serialized_traject_38_1_mini.json", "r"
        ) as f:
        ...
        # 3. Assert
        for section_expected, section_actual in zip(
            _expected_serialized_traject["dike_sections"],
            ...
            assert (
                section_expected["initial_assessment"]
                == section_actual["initial_assessment"]
            )
            assert section_expected["years"] == section_actual["years"]

        assert (
            _expected_serialized_traject["reinforcement_order_vr"]
            == _serialized_traject["reinforcement_order_vr"]
        )
        assert (
            _expected_serialized_traject["reinforcement_order_dsn"]
            == _serialized_traject["reinforcement_order_dsn"]
        )
        assert (
            _expected_serialized_traject["signalering_value"]
            == _serialized_traject["signalering_value"]
        )
        assert (
            _expected_serialized_traject["lower_bound_value"]
            == _serialized_traject["lower_bound_value"]
        )
    ...
    def test_get_run_optimization_ids(self):
        ...
        # 2. Define test
        id_vr, id_dsn = get_run_optimization_ids(
            vr_config, optimization_run_name="Basisberekening"
        )
