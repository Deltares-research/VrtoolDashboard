...
def get_reliability_for_each_step(database_path, measures_per_step):
    ...
    def restructure_reliability_per_step(reliability_per_step):
        ...
        for step in reliability_per_step:
            ...
            if MechanismEnum.get_enum(step['mechanism_name']) not in reliability_steps[step['step_number']][
                'reliability']:
                reliability_steps[step['step_number']]['reliability'][
                    MechanismEnum.get_enum(step['mechanism_name'])] = {'beta': [], 'time': []}
            if step['time'] not in reliability_steps[step['step_number']]['reliability'][
                MechanismEnum.get_enum(step['mechanism_name'])]['time']:
                reliability_steps[step['step_number']]['reliability'][MechanismEnum.get_enum(step['mechanism_name'])][
    ...
    with open_database(database_path) as db:
        reliability_values = (
            OptimizationStepResultMechanism.select(OptimizationStepResultMechanism.optimization_step_id,
                ...
                MechanismPerSection,
                on=(OptimizationStepResultMechanism.mechanism_per_section_id == MechanismPerSection.id)).join(
                Mechanism).join(
                OptimizationStep,
                on=(OptimizationStepResultMechanism.optimization_step_id == OptimizationStep.id)).where(
