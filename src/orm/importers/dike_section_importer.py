...
class DikeSectionImporter(OrmImporterProtocol):
    ...
    def _get_initial_assessment(
        self, section_id: int, active_mechanisms: list[str]
    ) -> dict:
        """
        ...
        for mechanism in active_mechanisms:
            ...
            _mechanism_per_section_id = MechanismPerSection.get(
                (MechanismPerSection.section == section_id)
                & (MechanismPerSection.mechanism == _mechanism_id)
            ).id

            _query_betas = (
                AssessmentMechanismResult.select(
                    AssessmentMechanismResult.time, AssessmentMechanismResult.beta
                )
                .where(
                    AssessmentMechanismResult.mechanism_per_section
                    == _mechanism_per_section_id
                )
                .order_by(AssessmentMechanismResult.time)
            )

            _initial_assessment[mechanism] = [
                row.beta for row in _query_betas if row.time in self.assessment_time
            ]

        # Add section results
        _query_betas = (
            AssessmentSectionResult.select(
                AssessmentSectionResult.time, AssessmentSectionResult.beta
            )
    ...
    def _get_coordinates(self, section_data: SectionData) -> list[tuple[float, float]]:
        ...
        _coordinates = self.traject_gdf[
            self.traject_gdf["section_name"] == _section_name
        ].geometry.iloc[0]
    ...
    def import_orm_without_measure(self, orm_model: SectionData) -> DikeSection:
        ...
        _dike_section.is_reinforced_veiligheidsrendement = (
            True  # TODO remove this argument?
        )
        ...
        _dike_section.revetment = (
            True if "Revetment" in _dike_section.active_mechanisms else False
        )

        _dike_section.initial_assessment = self._get_initial_assessment(
            orm_model, _dike_section.active_mechanisms
        )
        _dike_section.years = self.assessment_time

        # Set the final measures to the initial assessment, they will be modified later on.
        _dike_section.final_measure_veiligheidsrendement = self._get_no_measure_case(
            _dike_section.initial_assessment
        )
        _dike_section.final_measure_doorsnede = self._get_no_measure_case(
            _dike_section.initial_assessment
        )
    ...
    def _get_all_section_mechanism(self, section_data: SectionData) -> list[str]:
        ...
        mechanisms_for_section = (
            Mechanism.select(Mechanism.name)
            .join(
                MechanismPerSection,
                on=(Mechanism.id == MechanismPerSection.mechanism_id),
            )
