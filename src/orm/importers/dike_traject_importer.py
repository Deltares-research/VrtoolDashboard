...
class DikeTrajectImporter(OrmImporterProtocol):
    ...
    def _import_dike_section_list(
        self,
        orm_dike_section_list: list[SectionData],
    ...
    def parse_geo_dataframe(self, traject_name: str) -> GeoDataFrame:
        ...
        )  # Serialize the geometry column to a list of coordinates
    ...
    def import_orm(self, orm_model, path_database) -> DikeTraject:
        ...
        _traject_name = orm_model.DikeTrajectInfo.get(
            orm_model.DikeTrajectInfo.traject_name == self.traject_name
        ).traject_name
        _traject_id = DikeTrajectInfo.get(
            DikeTrajectInfo.traject_name == _traject_name
        ).id
        _traject_p_lower_bound = DikeTrajectInfo.get(
            DikeTrajectInfo.traject_name == _traject_name
        ).p_max
        _traject_p_signal = get_signal_value(_traject_p_lower_bound)
        _flood_damage = DikeTrajectInfo.get(
            DikeTrajectInfo.traject_name == _traject_name
        ).flood_damage
        ...
        _dike_traject.dike_sections = self._import_dike_section_list(
            _selected_sections, _traject_gdf
        )

        # import solution: both
        _solution_importer = TrajectSolutionRunImporter(
            dike_traject=_dike_traject,
