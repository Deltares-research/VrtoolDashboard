...
def _get_single_measure_type(opt_step: OptimizationStep) -> str:
    measure_type = (
        ...
        .join(OptimizationSelectedMeasure)
        .where(
            OptimizationSelectedMeasure.id == opt_step.optimization_selected_measure_id
        )
        .get()
    )
    return measure_type
...
def _get_combined_measure_name(optimization_step: OptimizationStep) -> str:
    if optimization_step.count() == 2:
        name = (
            _get_single_measure(optimization_step[0]).name
            + " + "
            + _get_single_measure(optimization_step[1]).name
        )
        ...
        name = (
            ...
            + _get_single_measure(optimization_step[1]).name
            + " + "
            + _get_single_measure(optimization_step[2]).name
        )
    else:
        raise ValueError()
...
def _get_combined_measure_type(optimum_step: OptimizationStep) -> list:
    if optimum_step.count() == 2:
        measure_type = [
            _get_single_measure_type(optimum_step[0]).name,
            _get_single_measure_type(optimum_step[1]).name,
        ]
...
def _get_combined_measure_investment_year(
...
def _get_measure_cost(optimization_steps: OptimizationStep) -> float:
    ...
    for optimum_step in optimization_steps:
        optimum_selected_measure = OptimizationSelectedMeasure.get(
            OptimizationSelectedMeasure.id
            == optimum_step.optimization_selected_measure_id
        )
        measure_result = MeasureResult.get(
            MeasureResult.id == optimum_selected_measure.measure_result_id
        )
        cost += MeasureResultSection.get(
...
def _get_measure_parameters(optimization_steps: OptimizationStep) -> dict:
    ...
    for optimum_step in optimization_steps:
        ...
        if (
            _params.get("transition_level") is None
            and params_transition_level.count() > 0
        ):
        ...
        if params_beta_target.count() > 0:
            ...
            _ini_measure_result = (
                ...
                .where(MeasureResult.measure_per_section_id == _measure_per_section_id)
                .order_by(MeasureResult.id.asc())
            )
            ...
            _params["pf_target_ratio"] = round(
                beta_to_pf(ini_beta_target[0].value)
                / beta_to_pf(_params["beta_target"]),
                1,
            )
            _params["diff_transition_level"] = (
                _params["transition_level"] - ini_transition_level[0].value
            )
...
def _get_measure(
    optimization_steps, active_mechanisms: list, assessment_time: list[int]
) -> dict:
    """
