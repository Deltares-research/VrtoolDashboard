...
from src.layouts.layout_traject_page.layout_download_buttons import (
...
df_project_summary_table = pd.DataFrame(
    columns=["project", "section_number", "start_year", "end_year" "length"], data=[]
)
...
layout_radio_helper_map_switch = dbc.RadioItems(
    ...
    options=[
        {
            "label": ProgramDefinitionMapType.SIMPLE.value,
            "value": ProgramDefinitionMapType.SIMPLE.name,
        },
        {
            "label": ProgramDefinitionMapType.PROJECTS.value,
            "value": ProgramDefinitionMapType.PROJECTS.name,
        },
        {
            "label": ProgramDefinitionMapType.ASSESSMENT_PROBABILITIES.value,
            "value": ProgramDefinitionMapType.ASSESSMENT_PROBABILITIES.name,
        },
        {
            "label": ProgramDefinitionMapType.VEILIGHEIDSRENDEMENT_INDEX.value,
            "value": ProgramDefinitionMapType.VEILIGHEIDSRENDEMENT_INDEX.name,
        },
    ],
    value="SIMPLE",
    inline=True,
...
left_side = [
    ...
    dcc.Upload(
        id="upload-dike-data",
        children=html.Div(["", html.A("Upload een json-bestand van een traject")]),
        style={
            "width": "100%",
            "height": "60px",
            "lineHeight": "60px",
                ...
                [
                    dmc.Group(
                        [
                            dmc.TextInput(
                                label="Naam project",
                                id=PROJECT_NAME_INPUT_FIELD_ID,
                                style={"width": "15%"},
                            ),
                            multi_select,
                            dmc.NumberInput(
                                id=PROJECT_START_YEAR_INPUT_FIELD_ID,
                                min=2025,
                                max=2125,
                                step=5,
                                label="Startjaar",
                            ...
                            dmc.NumberInput(
                                id=PROJECT_END_YEAR_INPUT_FIELD_ID,
                                min=2025,
                                max=2125,
                                step=5,
                                label="Eindjaar",
                            ...
                            dmc.Button(
                                "Verwijder project", id=DELETE_PROJECT_BUTTON_ID
                            ),
                        ],
                        align="top",
                    ),
