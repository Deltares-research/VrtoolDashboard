...
from src.layouts.layout_database_interaction.layout_custom_measures_table import (
    columns_defs,
)
...
def upload_csv_and_add_measure(
    contents: str, vr_config: dict
    ...
    custom_measure_names = list(
        set([measure["measure_name"] for measure in custom_measures_ini])
    )
...
def process(row_data, _vr_config, custom_measure_names):
    ...
    if custom_measure_list_1 == "Invalid error name":
        return (
            True,
            [
                "Onjuiste mechanism in het csv bestand, zorg ervroor dat deze correct is: Piping, Stabiliteit, Overslag, Bekleding"
            ],
            dash.no_update,
        )
    ...
    if (
        row_data[0][0].lower()
        != "maatregel,dijkvaknaam,mechanisme,tijd,kosten,betrouwbaarheidsindex"
    ):
        return (
            True,
            [
                "Onjuiste csv-header: zorg ervoor dat deze correct is: Maatregel,Dijkvaknaam,Mechanisme,Tijd,Kosten,Betrouwbaarheidsindex"
            ],
            dash.no_update,
        )
    ...
    def get_next_backup_filename(dir: Path):
        ...
        while True:
            ...
            backup_file_path = dir.joinpath(
                f"{_vr_config.input_database_name}_backup_{version_str}.db"
            )
    ...
    with ModalPopupLogHandler() as handler:
        _added_measures = add_custom_measures(_vr_config, custom_measure_list_1)

    # 5. Update table displaying ALL custom measures
    custom_measures = get_all_custom_measures(_vr_config)

    df = pd.DataFrame(
        columns=[col["field"] for col in columns_defs], data=custom_measures
    )
    ...
    for meas in custom_measure_list_1:
        ...
        if meas["MEASURE_NAME"] in custom_measure_names:
            children_list_to_display.append(
                f'{meas["MEASURE_NAME"]} zit al in de database en was niet toegevoegd.'
            )

        else:
            children_list_to_display.append(
                f'{meas["MEASURE_NAME"]} toegevoegd aan database.'
            )
...
def remove_all_custom_measure_in_db(n_clicks, vr_config: dict):
    ...
    custom_measures_ini = get_all_custom_measures(_vr_config)
    custom_measure_names_ini = list(
        set([measure["measure_name"] for measure in custom_measures_ini])
    )

    safe_clear_custom_measure(_vr_config)
    custom_measures = get_all_custom_measures(_vr_config)
    custom_measure_names_after = list(
        set([measure["measure_name"] for measure in custom_measures])
    )
...
def convert_custom_table_to_input(row_data: list) -> list[dict]:
    ...
    for row in row_data[1:]:
        ...
        if len(splitted_row) != 6:
            raise ValueError(
                f"Row {row} does not have 6 columns, found {len(splitted_row)} columns."
            )
...
def fill_custom_measures_table_from_database(
    vr_config: dict, dike_traject_data: dict
) -> list[dict, dict]:
    """
    ...
    new_columns_defs[1]["cellEditorParams"]["values"] = [
        section.name
        for section in DikeTraject.deserialize(dike_traject_data).dike_sections
    ]
