...
from src.component_ids import (
    BUTTON_DOWNLOAD_ASSESSMENT_NB_CLICKS,
    BUTTON_DOWNLOAD_OVERVIEW_NB_CLICKS,
    BUTTON_DOWNLOAD_REINFORCED_SECTIONS_NB_CLICKS,
    BUTTON_SAVE_RUN_AS_JSON,
    DOWNLOAD_ASSESSMENT_BUTTON_ID,
    DOWNLOAD_ASSESSMENT_ID,
    DOWNLOAD_OVERVIEW_BUTTON_ID,
    DOWNLOAD_OVERVIEW_ID,
    DOWNLOAD_REINFORCED_SECTIONS_BUTTON_ID,
    DOWNLOAD_REINFORCED_SECTIONS_ID,
    DOWNLOAD_RUN_JSON_ID,
    RUN_SAVE_NAME_ID,
...
def download_traject_run_json(
    dike_traject_data: dict, n_clicks: int, run_name: str, vr_config: dict
):
...
def download_overview_geojson(
    dike_traject_data: dict, n_clicks: int, store_n_click_button: int
) -> tuple[dict, int]:
    """
    ...
    if (
        ...
        return (
            dict(
                content=_dike_traject.export_to_geojson(params),
                filename="overzicht.geojson",
            ),
            n_clicks,
        )
...
def download_assessment_geojson(
    ...
    if (
        ...
        return (
            dict(
                content=_dike_traject.export_to_geojson(params),
                filename=f"beoordelingsresultaten_{selected_year}.geojson",
            ),
            n_clicks,
        )
...
def download_reinforced_sections_geojson(
    ...
    if (
        ...
        return (
            dict(
                content=_dike_traject.export_to_geojson(params),
                filename=f"versterkingsmaatregelen_{selected_year}_{calculation_type}.geojson",
            ),
            n_clicks,
        )
